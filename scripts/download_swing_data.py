...
def _write_yahoo_frame(symbol: str, frame: pd.DataFrame, destination: Path) -> bool:
    ...
    if isinstance(x.columns, pd.MultiIndex):
        # For a single ticker Yahoo can still return a two-level column index.
    ...
    if "adj_close" not in x.columns:
        # Keep execution close and use close as research price if Yahoo did not
        # provide an adjusted close field for this ticker.
        x["adj_close"] = x["close"]
