...
def simulate_trades(trades: pd.DataFrame, cfg: dict) -> Tuple[pd.DataFrame, float]:
    if trades.empty:
        return trades.assign(return=[]), 0.0
    ...
    return out, float(out["weighted_return"].sum()) if not out.empty else 0.0
...
def metrics(trades: pd.DataFrame, strategy: str, initial_capital: float, score: float = 0.0) -> BacktestResult:
    ...
    trade_r = trades["return"].astype(float)
    daily_r = trades.groupby("signal_date")["weighted_return"].sum().sort_index()
    wins, losses = trade_r[trade_r > 0], trade_r[trade_r <= 0]
    pf = float(wins.sum() / abs(losses.sum())) if len(losses) else float("inf")
    eq = (1 + daily_r).cumprod()
    dd = eq / eq.cummax() - 1
    sharpe = float(daily_r.mean() / daily_r.std() * math.sqrt(252)) if daily_r.std() > 0 else 0.0
    total = float(eq.iloc[-1] - 1)
    years = max((pd.to_datetime(daily_r.index).max() - pd.to_datetime(daily_r.index).min()).days / 365.25, 1 / 365.25)
    ann = float((1 + total) ** (1 / years) - 1) if 1 + total > 0 else -1.0
    turnover = float((trades["weight"] * 2).sum())
    stability = float((daily_r.rolling(63).mean().dropna() > 0).mean()) if len(daily_r) >= 63 else float((daily_r.mean() > 0))
    return BacktestResult(strategy, len(trade_r), float((trade_r > 0).mean()), pf, float(trade_r.mean()), total, ann, sharpe, float(dd.min()), turnover, stability, score)
...
def run_ml_walk_forward(df: pd.DataFrame, family: str, cfg: dict, top_n: int = 10) -> pd.DataFrame:
    ...
    while start < len(dates):
        ...
        if len(train) < cfg["research"].get("min_train_observations", 2000) or test.empty or val.empty:
            start += test_days
            continue
        if family == "ml_ranker":
...
def run(config_path: str) -> Dict[str, object]:
    ...
    def composite(r: BacktestResult) -> float:
        ...
        return (0.25 * np.tanh(max(r.sharpe, -3) / 2) + 0.20 * pf + 0.15 * np.tanh(r.expectancy * 100) + 0.15 * dd_quality + 0.15 * r.stability + 0.10 * np.tanh(max(r.annualized_return, -1) * 3))

    ranked = sorted(results, key=composite, reverse=True)
    leaderboard = [{**r.__dict__, "score": composite(r)} for r in ranked]
