...
def event_sim(selected, prices, h, cfg, initial_equity=1.0):
    """Event-driven simulation for one OOS fold.

    initial_equity is supplied by the caller so walk-forward folds form one
    continuous OOS account instead of independently resetting to 1.0.
    """
    if selected.empty:
    ...
    for dt in timeline:
        ...
        for p in openp:
            ...
            # Conservative gap handling: if the bar opens beyond a stop/target,
            # fill at the achievable open rather than the requested level.
            if stop_hit:
                raw_ex = float(b.open) if ((side == 1 and b.open <= p['stop']) or
                                           (side == -1 and b.open >= p['stop'])) else p['stop']
                ex = raw_ex * (1 - slip if side == 1 else 1 + slip)
                reason = 'stop'
            elif target_hit:
                raw_ex = float(b.open) if ((side == 1 and b.open >= p['target']) or
                                           (side == -1 and b.open <= p['target'])) else p['target']
                ex = raw_ex * (1 - slip if side == 1 else 1 + slip)
                reason = 'target'
            elif i - p['entry_i'] + 1 >= h:
                ex = float(b.close) * (1 - slip if side == 1 else 1 + slip)
                reason = 'time'
    ...
    # Force-close positions at the end of this OOS fold. The resulting equity
    # is carried into the next fold by run().
    if openp:
        dt = timeline[-1]
        for p in openp:
            g = groups[p['symbol']]
...
def metrics(trades):
    ...
    for h, g in trades.groupby('horizon'):
        g = g.sort_values(['exit_dt', 'symbol'])
        pnl = g.groupby('exit_dt').pnl.sum().sort_index()
        equity = 1.0 + pnl.cumsum()
        daily_ret = pnl / equity.shift(1).fillna(1.0)
        dd = equity / equity.cummax() - 1

        # Include zero-P&L business days so monthly returns do not silently
        # disappear when a month has no realized exits on a particular day.
        idx = pd.date_range(pnl.index.min(), pnl.index.max(), freq='B')
        full_pnl = pnl.reindex(idx, fill_value=0.0)
        full_equity = 1.0 + full_pnl.cumsum()
        full_ret = full_pnl / full_equity.shift(1).fillna(1.0)
        months = full_ret.groupby(full_ret.index.to_period('M')).apply(
            lambda z: (1 + z).prod() - 1)
        ...
        rows.append({
            ...
            'max_drawdown': dd.min(),
            'best_month': months.max(),
            'worst_month': months.min(),
...
def run(cfg):
    ...
    x = features(load(cfg), hs)
    dates = np.array(sorted(x.date.unique()))
    r = cfg['research']
    tn, vn, te, step = [int(r[k]) for k in ['train_days', 'validation_days', 'test_days', 'step_days']]
    seed = int(r['random_state'])
    sample = int(r['sample_per_fold'])
    k = int(r['n_clusters'][1])

    allp = []
    folds = []
    equity_by_h = {h: 1.0 for h in hs}

    for s in range(tn, len(dates) - vn - te + 1, step):
        fold_no = len(folds) + 1
        tr = x[x.date.isin(dates[s-tn:s])].dropna(
            subset=BASE[:-1] + [f'fwd_{max(hs)}']).copy()
        va = x[x.date.isin(dates[s:s+vn])].dropna(
            subset=BASE[:-1] + [f'fwd_{max(hs)}']).copy()
        ...
        tr = tr.dropna(subset=[f'fwd_{max(hs)}'])
        va = va.dropna(subset=[f'fwd_{max(hs)}'])
        if tr.empty or va.empty or tes.empty:
            continue
        tr, cols = frame(tr)
        va, _ = frame(va)
        tes, _ = frame(tes)
        model = LGBMRegressor(
            n_estimators=int(r['max_estimators']), learning_rate=.05,
        ...
        va['score'] = model.predict(va[cols])
        tes['score'] = model.predict(tes[cols])
        q = float(va.score.quantile(.75))
        if int((va.score >= q).sum()) < int(r['min_signal_obs']):
            q = float(va.score.quantile(.60))
        selected = tes[tes.score >= q].copy()

        fold_row = {
            'fold': fold_no, 'k': k, 'validation_threshold': q,
            'test_candidates': len(selected),
        }
        for h in hs:
            ...
            if not p.empty:
                p['fold'] = fold_no
                p['fold_start_equity'] = fold_start
                p['fold_end_equity'] = fold_end
                allp.append(p)
            fold_row[f'equity_start_h{h}'] = fold_start
            fold_row[f'equity_end_h{h}'] = fold_end
            fold_row[f'pnl_h{h}'] = fold_end - fold_start
    ...
    m = metrics(out)
    m.to_csv('docs/intraday_meta_v3_metrics.csv', index=False)
    print(m.to_string(index=False))


if __name__ == '__main__':
    ap = argparse.ArgumentParser()
    ap.add_argument('--config', default='config/intraday_meta_v2.yaml')
    a = ap.parse_args()
    run(yaml.safe_load(open(a.config)))
