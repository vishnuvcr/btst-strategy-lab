...
def make_features(x,h):
    ...
    if 'volume' in x:
        ...
        den=x.volume.fillna(0).groupby([x.symbol,x.date]).transform('sum').replace(0,np.nan);v=(x.close*x.volume.fillna(0)).groupby([x.symbol,x.date]).transform('sum')/den;x['vwap_gap']=v/x.close-1
    else:x['volume_z']=0.;x['vwap_gap']=0.
    x['realized_vol']=g['ret_1'].transform(lambda s:s.rolling(12,min_periods=6).std());x['accel']=g['ret_3'].transform(lambda s:s-s.shift(3));x['session_frac']=gs.cumcount()/gs.datetime.transform('size')
    x['atr_pct']=g.apply(lambda z: ((z.high-z.low).rolling(20,min_periods=10).mean()/z.close)).reset_index(level=0,drop=True)
    x['next_open']=gs.open.shift(-1);x[f'fwd_{h}']=gs.close.shift(-h)/x.next_open-1
    # Same-session opening range, prior to the current bar only.
    for b in [3,6,12]:
        hi=gs.high.transform(lambda s:s.shift(1).rolling(b,min_periods=b).max());lo=gs.low.transform(lambda s:s.shift(1).rolling(b,min_periods=b).min());x[f'orb{b}_hi']=hi;x[f'orb{b}_lo']=lo
    return x.replace([np.inf,-np.inf],np.nan)

def signals(x):
    out=[]
    for name in STRATEGIES:
        z=x[['datetime','date','symbol']+BASE_FEATURES+['close','high','low','next_open']+[c for c in x.columns if c.startswith('orb')]+[c for c in x.columns if c.startswith('fwd_')]+['atr_pct']].copy();z['strategy']=name;z['strategy_id']=STRATEGIES.index(name)
        ...
        # Avoid weak/no-op candidates.
        z=z[z.action!=0].copy();out.append(z)
    return pd.concat(out,ignore_index=True) if out else pd.DataFrame()

def add_clusters(train,val,test,k,seed):
    sc=RobustScaler().fit(train[BASE_FEATURES[:-1]]);km=KMeans(n_clusters=k,n_init=10,random_state=seed).fit(sc.transform(train[BASE_FEATURES[:-1]]))
    centers=km.cluster_centers_
    for z in [train,val,test]:
        z['cluster']=km.predict(sc.transform(z[BASE_FEATURES[:-1]]));a=sc.transform(z[BASE_FEATURES[:-1]]);c=centers[z.cluster.to_numpy()];z['cluster_conf']=1/(1+((a-c)**2).sum(axis=1))
...
def model_frame(z):
    ...
    cols=BASE_FEATURES+[f'strat_{i}' for i in range(len(STRATEGIES))]+['side','ret_x_vol','cluster_x_side']
    return q,cols

def portfolio(test,score_col,h,cfg):
    cost=2*(float(cfg['costs']['slippage_bps_per_side'])+float(cfg['costs']['transaction_cost_bps_per_side']))/10000;maxpos=int(cfg['portfolio']['max_positions']);gross=float(cfg['portfolio']['max_gross_exposure'])
    stop=float(cfg['execution']['stop_atr']);target=float(cfg['execution']['target_atr']);mh=min(int(cfg['execution']['max_holding_bars']),h)
    # One entry per timestamp, top confidence/model score; next-open execution.
    picks=test.sort_values(['datetime',score_col],ascending=[True,False]).groupby('datetime',sort=False).head(maxpos).copy()
    picks['weight']=gross/maxpos
    # Conservative executable return: next-open to h-th future close, with ATR stop/target barriers approximated from future extremes.
    fwd=picks[f'fwd_{h}'].to_numpy();atr=picks.atr_pct.fillna(0.01).clip(.005,.20).to_numpy();a=atr
    raw=picks.action.to_numpy()*fwd
    raw=np.where(raw<=-stop*a,-stop*a,raw);raw=np.where(raw>=target*a,target*a,raw)
    picks['ret']=raw-cost;picks['weighted']=picks.weight*picks.ret
    return picks

def run(cfg):
    x=make_features(load(cfg),int(cfg['research']['horizons_bars'][-1]));dates=np.array(sorted(x.date.unique()));r=cfg['research'];tn,vn,te,step=map(int,[r['train_days'],r['validation_days'],r['test_days'],r['step_days']]);seed=int(r['random_state']);sample=int(r['sample_per_fold']);outs=[];logs=[]
    for s in range(tn,len(dates)-vn-te+1,step):
        ...
        k=int(r['n_clusters'][1]);tr,va,tef=add_clusters(tr,va,tef,k,seed)
        tr=signals(tr);va=signals(va);tef=signals(tef)
        tr=tr.dropna(subset=['fwd_24']);va=va.dropna(subset=['fwd_24'])
        if tr.empty or va.empty or tef.empty:continue
        tr,cols=model_frame(tr);va,_=model_frame(va);tef,_=model_frame(tef)
        mdl=LGBMRegressor(n_estimators=int(r['max_estimators']),learning_rate=.05,num_leaves=31,max_depth=6,subsample=.8,colsample_bytree=.8,reg_lambda=2,random_state=seed,n_jobs=2,verbosity=-1)
        mdl.fit(tr[cols],tr.fwd_24);va['score']=mdl.predict(va[cols]);tef['score']=mdl.predict(tef[cols])
        # Validation selects a score quantile; no test-return tuning.
        ...
        for h in r['horizons_bars']:
            if f'fwd_{h}' not in selected:selected[f'fwd_{h}']=selected.groupby(['symbol','date'],group_keys=False).close.shift(-int(h))/selected.next_open-1
            p=portfolio(selected,'score',int(h),cfg);p['horizon']=int(h);p['fold']=len(logs)+1;outs.append(p)
        logs.append({'fold':len(logs)+1,'k':k,'validation_threshold':q,'validation_candidates':len(tv),'test_candidates':len(selected)})
    out=pd.concat(outs,ignore_index=True) if outs else pd.DataFrame();os.makedirs('docs',exist_ok=True);out.to_csv('docs/intraday_meta_v2_oos.csv',index=False);pd.DataFrame(logs).to_csv('docs/intraday_meta_v2_folds.csv',index=False)
    rows=[]
