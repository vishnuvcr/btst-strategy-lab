...
def features(x):
    ...
    x=x.copy(); x['ret_1']=g.close.pct_change(); x['ret_3']=g.close.pct_change(3); x['ret_6']=g.close.pct_change(6)
    ...
    if 'volume' in x:
        ...
        x['vwap']= (x.close*x.volume.fillna(0)).groupby([x.symbol,x.date]).transform('sum')/x.volume.fillna(0).groupby([x.symbol,x.date]).transform('sum').replace(0,np.nan); x['vwap_gap']=x.close/x.vwap-1
    else: x['volume_z']=0.; x['vwap_gap']=0.
    x['realized_vol']=g.ret_1.transform(lambda s:s.rolling(12,min_periods=6).std())
    x['accel']=x.ret_3-x.ret_3.shift(3)
    x['session_frac']=x.groupby(['symbol','date']).cumcount()/x.groupby(['symbol','date']).datetime.transform('size')
    # Executable future return: current close to next 3/6/12/24-bar close.
    for h in [3,6,12,24]: x[f'fwd_{h}']=g.close.shift(-h)/x.close-1
    x['fwd_6_label']=(x.fwd_6>0).astype(float)
    return x.replace([np.inf,-np.inf],np.nan)


def simulate(day, action, h, cost, max_pos, gross):
    if day.empty:return None
    picks=day[action!=0].copy()
    if picks.empty:return None
    # strongest confidence first; action sign determines direction.
    picks['strength']=picks['confidence'].abs(); picks=picks.sort_values('strength',ascending=False).head(max_pos)
    w=gross/len(picks); picks['ret']=picks.action*picks[f'fwd_{h}']-cost; picks['weighted']=w*picks.ret
    return picks


def run(cfg):
    x=features(load(cfg)); dates=np.array(sorted(x.date.unique())); r=cfg['research']; tn,vn,te,step=map(int,[r['train_days'],r['validation_days'],r['test_days'],r['step_days']]); seed=int(r['random_state']); cost=2*(float(cfg['costs']['slippage_bps_per_side'])+float(cfg['costs']['transaction_cost_bps_per_side']))/10000; max_pos=int(cfg['portfolio']['max_positions']); gross=float(cfg['portfolio']['max_gross_exposure']); sample=int(r['sample_per_fold']);
    outputs=[]; selections=[]
    for s in range(tn,len(dates)-vn-te+1,step):
        train=x[x.date.isin(dates[s-tn:s])].dropna(subset=FEATURES+['fwd_6']).copy(); val=x[x.date.isin(dates[s:s+vn])].dropna(subset=FEATURES+['fwd_6']).copy(); test=x[x.date.isin(dates[s+vn:s+vn+te])].dropna(subset=FEATURES).copy()
        if len(train)>sample: train=train.sample(sample,random_state=seed)
        scaler=RobustScaler().fit(train[FEATURES]); km=KMeans(n_clusters=int(r['n_clusters'][1]),n_init=10,random_state=seed).fit(scaler.transform(train[FEATURES])); train['cluster']=km.labels_; val['cluster']=km.predict(scaler.transform(val[FEATURES])); test['cluster']=km.predict(scaler.transform(test[FEATURES]))
        cg=val.groupby('cluster').fwd_6.agg(['count','mean','std']); eligible=cg[cg['count']>=int(r['min_pattern_obs'])].copy();
        if eligible.empty: continue
        longc=int(eligible['mean'].idxmax()); shortc=int(eligible['mean'].idxmin());
        # Select action threshold using validation economics, not test returns.
        lp=float(eligible.loc[longc,'mean']); sp=float(eligible.loc[shortc,'mean']);
        selections.append({'fold':len(selections)+1,'k':int(r['n_clusters'][1]),'long_cluster':longc,'short_cluster':shortc,'validation_long_edge':lp,'validation_short_edge':sp})
        for d,day in test.groupby('date',sort=True):
            ...
            # Confidence is distance/probability proxy; no future returns used for ranking.
            centers=km.cluster_centers_; z=scaler.transform(day[FEATURES]); dist=((z[:,None,:]-centers[None,:,:])**2).sum(axis=2); day['confidence']=1/(1+dist[np.arange(len(day)),day.cluster.to_numpy()])
            for h in [3,6,12,24]:
                q=simulate(day,day.action,h,cost,max_pos,gross)
                if q is not None:
                    q=q[['date','symbol','cluster','action','confidence',f'fwd_{h}','weight','ret','weighted']].copy(); q['horizon']=h; outputs.append(q)
    os.makedirs('docs',exist_ok=True); out=pd.concat(outputs,ignore_index=True) if outputs else pd.DataFrame(); out.to_csv('docs/intraday_unsupervised_meta_oos.csv',index=False); pd.DataFrame(selections).to_csv('docs/intraday_unsupervised_meta_selection.csv',index=False)
    rows=[]
